...
class PatternedLoader:
    ...
    def get_matter(self, matter_name="lattices"):
        ...
        times_s, pots_s = self.short_lattice.get_lattice_dynamics()

        pos_l = np.arange(-6,12,6)
        pos_s = np.arange(-6,8,2)
        barriers = []
        for pos in pos_l:
        ...
        for pos in pos_s:
            poss = pos*np.ones(len(times_s))
            wids = 0.5*np.ones(len(times_s))
            barr = self.qmf.create_barrier(positions=poss,
                                     heights=pots_s,
                                     widths=wids,
                                     times=times_s,
